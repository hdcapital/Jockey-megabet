...
Every response fetched through :class:`ArchivingClient` is persisted to
``data/raw/`` so that any number displayed by the scanner can later be
reproduced from the exact bytes the source returned — and so that a parse
failure can be reported with a path to the payload that caused it instead of
a shrug.

Reused, unchanged in substance, from the Jockey-Megabet scanner where this
layer was exercised against live Sportsbet and Betfair endpoints.
"""
